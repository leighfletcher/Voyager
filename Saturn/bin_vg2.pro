files=file_search('VGIRIS_0002_peer_review/DATA/SATURN_VG2/*_LSB.DAT')

     
     
;READ IN VOYAGER 2 NESR SPECTRUM
nwaven=file_lines('/home/jupiter/plan/fletcher/projects/IRIS/IRISdata/PDS/saturn/VGIRIS_0002_peer_review/CALIB/SATURN_VG2_NESR.TAB')
tmp=fltarr(2,nwaven)
openr,1,'/home/jupiter/plan/fletcher/projects/IRIS/IRISdata/PDS/saturn/VGIRIS_0002_peer_review/CALIB/SATURN_VG2_NESR.TAB'
readf,1,tmp
close,1
nesr = tmp(1,*)
...
for ifile=0,nfiles-1 do begin
    ...
    ;for i=0,nrecords-1 do begin
		...
		abs(mRecord.q9_latitude) le 90.0) then begin
               mderiv[c]=maxderiv 
               data[c]=mRecord
               c=c+1
...
set_plot,'ps'
device,filename='saturn_vg2irislat.eps',/encapsulated
!p.font=0
pcircle
loadct,0
...
plot,[ca_v2-3.,ca_v2+1.],[-90,90],/nodata,xs=1,ys=1,$
title='Voyager 2 Latitude Coverage',ytitle='Planetographic Latitude',$
xtitle='Date',$
...
set_plot,'ps'
device,filename='saturn_v2spx.eps',encapsulated=0
!p.font=0
pcircle
loadct,0
count=0
...
for ilat=0,nlats-1 do begin
    ...
    mlat=latS+ilat*lat_step
    file=strcompress('vg2_iris_'+string(mlat)+'.spx',/remove_all)
    openw,1,file
